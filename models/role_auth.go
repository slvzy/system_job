...
func (ra *RoleAuth) TableName() string {
	return TableName("user_role_auth")
}
...
func RoleAuthGetById(id int) ([]*RoleAuth, error) {
	...
	query := orm.NewOrm().QueryTable(TableName("user_role_auth"))
...
func RoleAuthDelete(id int) (int64, error) {
	_, err := orm.NewOrm().Raw("DELETE FROM `system_user_role_auth` WHERE `role_id` = ?",
