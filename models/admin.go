...
func AdminGetByName(loginName string) (*Admin, error) {
	a := new(Admin)
	err := orm.NewOrm().QueryTable(TableName("user_admin")).Filter("login_name", loginName).One(a)
	if err != nil {
		return nil, err
	}
	return a, nil
...
func AdminGetList(page, pageSize int, filters ...interface{}) ([]*Admin, int64) {
	...
	list := make([]*Admin, 0)
	query := orm.NewOrm().QueryTable(TableName("user_admin"))
	if len(filters) > 0 {
		l := len(filters)
		for k := 0; k < l; k += 2 {
...
func AdminGetById(id int) (*Admin, error) {
	r := new(Admin)
	err := orm.NewOrm().QueryTable(TableName("user_admin")).Filter("id", id).One(r)
	if err != nil {
		return nil, err
	}
	return r, nil
