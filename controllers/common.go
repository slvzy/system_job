...
func (self *BaseController) Auth() {
	...
	if len(arr) == 2 {
		...
		if userId > 0 {
			...
			noAuth := "ajaxsave/table/loginin/loginout/getnodes/start/apitask/apistart/apipause/ajaxtestserver"
			isNoAuth := strings.Contains(noAuth, self.actionName)
...
//项目信息
func productGroupLists() (sgl map[int]string) {
	Filters := make([]interface{}, 0)
	Filters = append(Filters, "status", 1)
	groupResult, n := models.ConfigTypeGetList(1, 1000, Filters...)
	sgl = make(map[int]string, n)
	for _, gv := range groupResult {
		sgl[gv.Id] = gv.Name
	}
